...
spreadLevelPlot.lm <- function(x, robust.line=TRUE, 
	...
	pch=1, col=palette()[1], col.lines=palette()[2:3], lwd=2, grid=TRUE, 
    id=FALSE, smooth=TRUE, ...){
    id <- applyDefaults(id, defaults=list(method=list("x", "y"), n=2, cex=1, col=palette()[1], location="lr"), type="id")
    if (isFALSE(id)){
        id.n <- 0
        id.method <- "none"
