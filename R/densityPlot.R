...
densityPlot.formula <- function(formula, data=NULL, subset, na.action=NULL, xlab, ylab, legend=TRUE, ...){
    m <- match.call(expand.dots = FALSE)
    if (is.matrix(eval(m$data, parent.frame()))) 
        m$data <- as.data.frame(data)
    m$legend <- m$xlab <- m$ylab <- m$... <- NULL
    ...
    if (length(formula) == 3){
        ...
        g <- mf[, -response]
        densityPlot(mf[[response]], g, xlab=xlab, ylab=ylab, 
