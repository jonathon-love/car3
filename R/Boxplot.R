...
Boxplot.default <- function(y, g, id=TRUE, xlab, ylab, ...){
    if (isFALSE(id)) {
        ...
        id <- applyDefaults(id, defaults=list(method="y", n=10, location="lr"), type="id")
    ...
    if (missing(g)) {
        ...
        if (id.method == "none" | id.n == 0) 
            ...
            if (id.location == "lr") text(at, c(y.low, y.high), labs, pos = 2, xpd=TRUE)
        ...
        if (id.method == "none" | id.n == 0) 
            ...
            if (length(b$out) > 0) {
                ...
                for (group in groups) {
                    ...
                    labs <- c(labs.low, labs.high)
                    if (id.location == "lr") text(at, c(y.low, y.high), labs, pos = pos, xpd=TRUE)
                    else maptools::pointLabel(c(at, at), c(y.low, y.high, y.low, y.high), 
                                                   c(paste0(" ", labs, " "), rep(" ", length(labs))), 
                                                   xpd=TRUE)
...
Boxplot.data.frame <-  function(y, id=TRUE, ...){
    if (isFALSE(id)) {
        ...
        id <- applyDefaults(id, defaults=list(method="y", n=10, location="lr", labels=rownames(y)), type="id")
        id.method <- match.arg(id$method, c("y", "identify", "none"))
        id.n <- id$n
        id.location <- id$location
        labels <- rep(id$labels, ncol(y))
    }
  Boxplot(as.list(y), id=list(method=id.method, n=id.n, location=id.location, labels=labels), ...)
}
