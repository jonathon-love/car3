...
scatterplotMatrix.formula <- function (formula, data=NULL, subset, id=FALSE, ...) {
    ...
    if (!groups) scatterplotMatrix(X, id=list(method=id.method, n=id.n, labels=labels, 
                                              cex=id.cex, col=id.col, location=id.location), ...)
    else{
        ncol<-ncol(X)
        scatterplotMatrix.default(X[, -ncol], groups=X[, ncol], id=list(method=id.method, n=id.n, labels=labels, 
                                                                        cex=id.cex, col=id.col, location=id.location), ...)
    }
}

scatterplotMatrix.default <- function(x, smooth=TRUE, id=FALSE, legend=TRUE, var.labels=colnames(x), 
    diagonal=c("density", "boxplot", "histogram", "oned", "qqplot", "none"), adjust=1, nclass,
    plot.points=TRUE, spread = !by.groups, reg.line=lm,
    transform=FALSE, family=c("bcPower", "yjPower"),
    ellipse=FALSE, groups=NULL, by.groups=FALSE, 
    use=c("complete.obs", "pairwise.complete.obs"),
    col=if (n.groups == 1) palette()[3:1] else rep(palette(), length=n.groups),
    pch=1:n.groups, lwd=1, lty=1, 
    cex=par("cex"), cex.axis=par("cex.axis"), cex.labels=NULL, cex.main=par("cex.main"), 
    row1attop=TRUE, ...){
    ...
    id <- applyDefaults(id, defaults=list(method="mahal", n=2, cex=1, col=palette()[1], location="lr"), type="id")
    if ("identify" %in% id$method) stop("interactive point identification not permitted") 
    if (isFALSE(id)){
        id.n <- 0
        id.method <- "mahal"
        labels <- id.cex <- id.col <- id.location <- NULL
    ...
    ellipse <- applyDefaults(ellipse, defaults=list(levels=c(0.5, 0.95), robust=TRUE), type="ellipse")
    if (isFALSE(ellipse)){
        levels <- NULL
        robust <- NULL
    }
